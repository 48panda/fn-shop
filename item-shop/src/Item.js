...
export class FortniteItem extends Component {
    render() {
        ...
        let flags = nda.flags || {}
        return (<div  className={`${this.props.owned ? "owned " : ""}item ${this.props.data.tileSize} ${(this.props.data.items[0].series || {}).backendValue || this.props.data.items[0].rarity.value}${flags.bIsCreatorCollabSeries ? " doIcon" : ""}${flags.bIsDCUSeries ? " doNoise" : ""}${flags.bIsPlatformSeries ? " doGamingLegends" : ""}${flags.bIsMarvelSeries ? " doMarvel" : ""}${flags.IsCharacter!==false?" character":""}`} style={{
            ...
            "--spotlight-strength":nda.scalings.Spotlight_Intensity || 0,
